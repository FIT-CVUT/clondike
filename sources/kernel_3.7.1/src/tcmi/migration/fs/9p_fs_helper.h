...
static int chroot_to(const char* path_to_chroot) {
	...
	//struct nameidata nd;
	struct path p;
	//Function path_lookup() is replaced to kern_path() with last parameter pointer to struct path

	err = kern_path(path_to_chroot, LOOKUP_FOLLOW | LOOKUP_DIRECTORY ,&p);
	if ( err ) {
	...
	/* Chroots.. duplicated code from chroot as it cannot be used directly */
        spin_lock(&current->fs->lock);		//Replaced write_lock to spin_lock (write_unlock -> spin_unlock) because in struct fs_struct type of lock was changed to spinlock_t | by Jiri Rakosnik
