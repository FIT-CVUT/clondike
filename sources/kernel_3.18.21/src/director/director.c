...
int director_task_fork(struct task_struct *parent, struct task_struct *child) {
	//msleep(1); /* this is not pretty - wait for other threads if destroy child */
	...
	mdbg(INFO4,"director_task_fork_1");
	if (child->nonmigratable) return 0;

	mdbg(INFO4,"director_task_fork_2");
	
	if (parent == NULL)	
		mdbg(INFO4,"director_task_fork_2.5");

	if ( is_director_pid(parent->pid) )
	    return 0;
	mdbg(INFO4,"director_task_fork_3");
