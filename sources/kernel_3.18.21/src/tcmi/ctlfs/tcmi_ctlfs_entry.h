...
static inline void tcmi_ctlfs_entry_put(struct tcmi_ctlfs_entry *self)
{
	if (self) {
		...
		mdbg(INFO4, "Decrementing ref. count of '%s'(%p) (dentry: %p).  Flags = %x", 
		     tcmi_ctlfs_entry_name(self), self, self->dentry, self->dentry->d_flags); //Removed function atomic_read for bad usage | by Jiri Rakosnik
