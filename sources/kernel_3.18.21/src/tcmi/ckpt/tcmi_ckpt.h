				       ...
				       vfs_method_t *vfs_method)
{
	...
	mdbg(INFO3, "DEBUG read vfs method return = %d should be count = %d", result, count);
	set_fs(old_fs);
					      ...
					      unsigned long flags)
{
	...
	addr = do_mmap_pgoff(self->file, addr, len, prot, flags, self->file->f_pos, &populate);  //Replaced function do_mmap to do_mmap_pgoff for kernel 3.7.x by Jiri Rakosnik
