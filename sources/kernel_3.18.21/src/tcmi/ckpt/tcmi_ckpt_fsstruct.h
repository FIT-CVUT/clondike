...
static inline int tcmi_ckpt_fsstruct_read(struct tcmi_ckpt *ckpt, struct task_struct* task)
{
	...
	char* pwd = NULL;
	struct path path;                           //Changed struct nameidata to struct path for function kern_path and another  | by Jiri Rakosnik
	...
	//path_get(&nd.path);
	task->fs->pwd = path;	              // Changed nd.path to path | by Jiri Rakosnik
	//path_put(&nd.path);
