...
static int mount_proxyfs(struct kkc_sock* sock) {	
	...
	mdbg(INFO2, "Going to mount proxyfs on dev: %s fsuid: %d euid: %d csa: %d euid: %d uid: %d, suid: %d", buffer, current_fsuid().val, current_euid().val, capable(CAP_SYS_ADMIN), current_euid().val, current_uid().val, current_suid().val );
...
static char proc_path[] = "/mnt/local/proc";
...
static int tcmi_migcom_migrated_task(void *data)
{
	...
	new_cred->uid.val = new_cred->euid.val = new_cred->suid.val = new_cred->fsuid.val = 
	new_cred->gid.val = new_cred->egid.val = new_cred->sgid.val = new_cred->fsgid.val = 999;
