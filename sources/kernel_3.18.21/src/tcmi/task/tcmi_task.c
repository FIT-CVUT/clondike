...
int stub_execve_clondike(const char *filename, const char *const argv[], const char *const envp[]);

static inline int copyof_kernel_execve(const char *filename, const char * const argv[], const char * const envp[]) {
	return stub_execve_clondike(filename, argv, envp);
...
memory_sanity_check("Sanity check on execve - just before the call");
	if ((err = copyof_kernel_execve((const char*)self_tsk->ckpt_pathname, (const char* const *)self_tsk->argv,
	/* if ((err = execve(self_tsk->ckpt_pathname, self_tsk->argv, */
			 (const char* const *)self_tsk->envp)) < 0) {
