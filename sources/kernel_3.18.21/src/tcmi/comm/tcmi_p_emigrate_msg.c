...
struct tcmi_msg* tcmi_p_emigrate_msg_new_tx(struct tcmi_slotvec *transactions, 
						       pid_t reply_pid, char *exec_name, char *ckpt_name, kuid_t euid, kgid_t egid, kuid_t fsuid, kgid_t fsgid, unsigned long jif)
{
	struct tcmi_p_emigrate_msg *msg;
	...
	msg->pid_and_size.euid = euid.val;
	msg->pid_and_size.egid = egid.val;
	msg->pid_and_size.fsuid = fsuid.val;
	msg->pid_and_size.fsgid = fsgid.val;
