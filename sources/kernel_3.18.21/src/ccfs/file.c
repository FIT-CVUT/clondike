...
int ccfs_iterate (struct file *filp, struct dir_context *dir_context) {
	int rc;
	...
	rc = iterate_dir(lower_file, dir_context);
...
static ssize_t ccfs_generic_file_read_iter(struct kiocb *iocb, struct iov_iter *iter) {
  ssize_t res;
  res = generic_file_read_iter(iocb, iter);
  mdbg(INFO3,"Read Iter ..." );  
  return res;
}

const struct file_operations ccfs_dir_fops = {
	.iterate = ccfs_iterate,
	.compat_ioctl = ccfs_ioctl,		
	.mmap = ccfs_file_mmap,
	.open = ccfs_open,
	.flush = ccfs_flush,
...
const struct file_operations ccfs_main_fops = {
	...
	.read_iter = ccfs_generic_file_read_iter,
	.write = do_sync_write,
