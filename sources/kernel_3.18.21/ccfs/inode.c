...
int ccfs_truncate(struct dentry *dentry, loff_t new_length)
{
	...
	/*Commented vmtruncate is deprecated, here is possible problem, by Jiri Rakosnik*/
	/*vmtruncate(inode, new_length);*/
	/*vmtruncate(lower_dentry->d_inode,
			new_length);*/
	
	/*Replace vmtruncate by truncate_setsize*/
	truncate_setsize(inode, new_length);
	truncate_setsize(lower_dentry->d_inode, new_length);
...
static int ccfs_setattr(struct dentry *dentry, struct iattr *ia)
{
	...
	rc = notify_change(lower_dentry, ia, NULL);
out:
...
static int ccfs_getattr(struct vfsmount *mnt, struct dentry *dentry, struct kstat *stat)
{
	...
	struct ccfs_inode* inode = ccfs_inode_to_private(dentry->d_inode);
	struct vfsmount *lower_mnt = ccfs_dentry_to_nested_mnt(dentry);
	struct path ccfs_lower_path = { .mnt = lower_mnt, .dentry = lower_dentry };
	...
	rc = vfs_getattr(&ccfs_lower_path, stat);
	...
	mdbg(INFO3, "Get stat returned res: %d uid %d gid %d", rc, stat->uid.val, stat->gid.val);
