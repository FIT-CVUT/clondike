 ...
 * memory(enforced by mount_single()), so only a reference counter of
...
void tcmi_ctlfs_put_root(void)
{
	...
	//Replaced construction reading mnt_count which it isn't included in vfsmount structure to function may_umount which return 1 when fs may be umounted | by Jiri Rakosnik
  if(may_umount(self.vfs_mnt))     
	self.vfs_mnt = NULL;
