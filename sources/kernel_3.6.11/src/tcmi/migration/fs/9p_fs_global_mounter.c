...
static int global_9p_mount(struct fs_mounter* mounter, int16_t fsuid, int16_t fsgid) {	
	...
	//struct nameidata nd;	Replaced to struct path for use function kern_path() instead of  path_lookup()
	struct path p;
	get_mount_root(mounter->params.mount_device, fsuid, fsgid, mount_buffer);
	err = kern_path(mount_buffer, LOOKUP_FOLLOW | LOOKUP_DIRECTORY ,&p);
	if ( err ) {
	...
	err = kern_path(root_buffer, LOOKUP_FOLLOW | LOOKUP_DIRECTORY ,&p);	
		if ( err ) {
